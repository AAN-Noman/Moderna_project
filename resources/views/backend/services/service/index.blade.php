    ...
    <div class="container">
        <div class="row">
            <div class="col-md-14">
                <div class="x_panel">
                    <div class="x_title">
                        <h2>All Services <a href="{{ route('backend.service.create') }}" class='btn btn-primary btn-sm'>Add Services</a> </h2>
                        ...
                        <table class="table table-striped jambo_table bulk_action">
                            <thead>
                                <tr class="headings">
                                    ...
                                    <th class='colume-title'>Icon Color</th>
                            ...
                            <tbody>
                                @forelse ($datas as $data)
                                    <tr class="even pointer">
                                        ...
                                        <td>{{ $data->iconColor }}</td>
                                        <td>{{ $data->icon }}</td>
                                        <td>{{ $data->status == 1 ? 'Active' : 'Deactive' }}</td>
                                        <td class="last">
                                            <a href="{{ route('backend.service.status', $data->id) }}"
                                                class="btn btn-{{ $data->status == 1 ? 'warning' : 'success' }} btn-sm">
                                                {{ $data->status == 1 ? 'Deactive' : 'Active' }}
                                            </a>
                                            <a href="{{ route('backend.service.edit', $data->id) }}"
                                                class="btn btn-primary btn-sm">View/Edit</a>
                                            <form class='d-inline'
                                                action="{{ route('backend.service.destroy', $data->id) }}" method='POST'>
                                                @csrf
                                                @method('DELETE')
                            ...
                            <tbody>
                                @forelse ($DataTranshed as $data)
                                    <tr class="even pointer">
                                        ...
                                        <td>{{ $data->status == 1 ? 'Active' : 'Deactive' }}</td>
                                        <td class="last">
                                            <a href="{{ route('backend.service.restore', $data->id) }}"
                                                class="btn btn-primary btn-sm">Restore</a>

                                            <button id="delete" value="{{ route('backend.service.hardDelete', $data->id) }}" class="btn btn-danger btn-sm">Hard Delete</button>
                                    </tr>
                                @empty
                                @endforelse
                            </tbody>
